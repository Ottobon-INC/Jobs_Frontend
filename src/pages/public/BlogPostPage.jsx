...
const BlogPostPage = () => {
    ...
    if (error || !post) return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-[#F4F1EA]">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-xl bg-white border border-zinc-100 p-12 rounded-[3rem] shadow-xl">
                ...
                <h1 className="text-4xl font-bold text-[#1C1A17] tracking-tight mb-4">Insight Not Found</h1>
                <p className="text-sm font-bold text-zinc-400 uppercase tracking-[0.2em] mb-12">{error || "The requested analysis is unavailable."}</p>
                <Link to="/blogs" className="inline-block bg-[#D45B34] text-white px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 hover:bg-[#B84A27] transition-all shadow-xl shadow-[#D45B34]/20">
                    Return to Feed
                </Link>
            </motion.div>
        </div>
    ...
    return (
        <div className="min-h-screen bg-[#F4F1EA] pb-24">
            {/* Minimal Header */}
            <div className="bg-[#222222] py-4 px-6 md:px-12 flex items-center justify-between border-b border-white/5 sticky top-0 z-50 backdrop-blur-md bg-[#222222]/90">
                <Link 
                    to="/blogs" 
                ...
                <div className="flex items-center gap-2 text-white/90">
                    <Sparkles size={14} className="text-[#D45B34]" />
            ...
            <article className="max-w-4xl mx-auto mt-20 px-6">
                ...
                >
                    <div className="inline-flex items-center gap-3 text-[10px] font-black text-[#1C1A17]/50 mb-10 bg-white px-8 py-3 rounded-full border border-zinc-100 shadow-sm uppercase tracking-widest">
                        <Calendar size={14} />
                    ...
                    <h1 className="text-4xl md:text-6xl font-bold mb-12 tracking-tight text-[#1C1A17] leading-[1.1]">
                        {post.title}
                    </h1>
                ...
                <motion.div
                    ...
                    prose-headings:text-[#1C1A17] prose-headings:font-bold prose-headings:tracking-tight
                    prose-p:text-zinc-600 prose-p:font-medium prose-p:leading-[1.8]
                    prose-strong:text-[#1C1A17]
                    prose-blockquote:border-l-4 prose-blockquote:border-[#D45B34] prose-blockquote:bg-white prose-blockquote:rounded-3xl prose-blockquote:py-10 prose-blockquote:px-12 prose-blockquote:not-italic prose-blockquote:text-[#1C1A17] prose-blockquote:font-medium
                "
                >
                    {post.summary && (
                        <div className="text-xl md:text-2xl font-bold text-[#1C1A17] leading-relaxed mb-12 border-l-4 border-[#D45B34]/30 pl-10 py-2">
                            {post.summary}
                        </div>
                    )}
                    ...
                    {post.url && (
                        <div className="mt-20 p-10 bg-[#222222] text-white rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-[rgba(28,26,23,0.2)]">
                            <div className="text-center md:text-left">
                                <h3 className="font-bold text-white text-2xl m-0 tracking-tight">Source Context</h3>
                                <p className="text-sm font-medium text-[#F4F1EA]/75 m-0 mt-2">Proceed to the root source to decrypt the full context of this signal.</p>
                            </div>
                            <a href={post.url} target="_blank" rel="noopener noreferrer" className="shrink-0 flex items-center gap-3 bg-white text-[#1C1A17] px-10 py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:scale-105 hover:bg-[#F4F1EA] transition-all shadow-xl no-underline">
                ...
                <div className="mt-32 p-12 bg-white border border-zinc-100 rounded-[3rem] flex flex-col md:flex-row justify-between items-center gap-8 shadow-sm">
                    <div>
                        <p className="text-2xl font-bold text-[#1C1A17] tracking-tight flex items-center gap-3">
                    ...
                    <button className="flex items-center gap-3 px-10 py-5 bg-[#D45B34] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 hover:bg-[#B84A27] transition-all shadow-lg active:scale-95">
                        Copy Signal Link
                    </button>
                </div>
            </article>

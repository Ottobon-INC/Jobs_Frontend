  ...
  return (
    <div className="p-6 sm:p-10 bg-[#F4F1EA] min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-bold text-[#1C1A17]">Manage Rewards</h1>
          <p className="text-[#1C1A17]/60 font-medium">Create and manage vouchers, boosts, and exclusive perks</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#1C1A17]/15">
          <p className="text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1">Active Items</p>
          <p className="text-3xl font-bold text-[#1C1A17]">{stats.active_reward_items}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#1C1A17]/15">
          <p className="text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1">Total Redemptions</p>
          <p className="text-3xl font-bold text-[#1C1A17]">{stats.total_redemptions}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#1C1A17]/15">
          <p className="text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1">Users Participated</p>
          <p className="text-3xl font-bold text-[#1C1A17]">{stats.total_users_with_rewards}</p>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#F4F1EA]/50 border-b border-[#1C1A17]/15">
                <th className="px-6 py-4 text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest">Name</th>
                <th className="px-6 py-4 text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest">Category</th>
                <th className="px-6 py-4 text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest">Cost</th>
                <th className="px-6 py-4 text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest">Stock</th>
                <th className="px-6 py-4 text-xs font-bold text-[#1C1A17]/40 uppercase tracking-widest">Status</th>
            ...
            <tbody className="divide-y divide-[#1C1A17]/10">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-[#F4F1EA]/20 transition-colors">
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-[#1C1A17]">{item.name}</p>
                  ...
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-[#1C1A17]">{item.cost.toLocaleString()}</p>
                  </td>
                  <td className="px-6 py-4 text-sm font-medium text-[#1C1A17]/60">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      <label className="block text-[10px] font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1.5 ml-1">Reward Name</label>
                      <input
                        type="text"
                        required
                        value={formData.name}
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-[10px] font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1.5 ml-1">Description</label>
                      <textarea
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1.5 ml-1">Category</label>
                      <select
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1.5 ml-1">Coin Cost</label>
                      <input
                        type="number"
                        required
                        min="1"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1.5 ml-1">Expiry (Days)</label>
                      <input
                        type="number"
                        min="1"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#1C1A17]/40 uppercase tracking-widest mb-1.5 ml-1">Stock (Empty for ∞)</label>
                      <input
                        type="number"

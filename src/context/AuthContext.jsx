...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
            // Set loading to true while we resolve the new identity
            setLoading(true);
    ...
    const fetchProfile = async (retries = 3, cachedUser = null) => {
        try {
    ...
    const handleAuthSuccess = async (token) => {
        // Manually sync immediately so the UI doesn't have to wait for onAuthStateChange
    ...
    }), [session, user, profile, role, savedJobIds, loading, fetchSavedJobs, logout]);
